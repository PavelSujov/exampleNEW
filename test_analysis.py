...
def empty_dataframe():
    ...
    columns = [
        'Материал пластины', 'Тип резки', 'Толщина пластины, мкм',
        'Ширина реза, мкм', 'Сколы лицевая сторона (медиана), мкм',
        'Сколы обратная сторона (медиана), мкм', 'Производительность, шт/час',
        'Срок службы диска, резов', 'Скорость подачи, мм/с',
        'Частота оборотов шпинделя, об/мин', 'Артикул диска'
    ]
...
class TestFilterData:
    ...
    def test_filter_data_basic(self, sample_dataframe):
        ...
        assert all((result['Толщина пластины, мкм'] >= 300) & (result['Толщина пластины, мкм'] <= 400))
        assert all((result['Ширина реза, мкм'] >= 30) & (result['Ширина реза, мкм'] <= 35))
...
class TestGetThicknessRanges:
    ...
    def test_get_thickness_ranges_single_value(self):
        ...
        single_df = pd.DataFrame({
            'Толщина пластины, мкм': [300, 300, 300]
        })
